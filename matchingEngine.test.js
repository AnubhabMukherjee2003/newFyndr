// matchingEngine.test.js — run: node matchingEngine.test.js
...
function assertEqual(a, b, m) { if (JSON.stringify(a) !== JSON.stringify(b)) throw new Error(`${m ?? ""}\n     expected: ${JSON.stringify(b)}\n     got:      ${JSON.stringify(a)}`); }
function assertClose(a, b, tol=0.05, m) { if (Math.abs(a-b)>tol) throw new Error(`${m??""} expected≈${b} got ${a}`); }
function assertThrows(fn, msg) {
  try { fn(); throw new Error("Expected an error but none was thrown"); }
  catch (e) { if (e.message === "Expected an error but none was thrown") throw e; }
}


// ── Enums ─────────────────────────────────────────────────────
console.log("\n── Enums ────────────────────────────────────────────────────────");
test("DataType frozen",  () => assert(Object.isFrozen(DataType)));
test("MatchType frozen", () => assert(Object.isFrozen(MatchType)));
test("InputType frozen", () => assert(Object.isFrozen(InputType)));
...
test("createUser returns user with id, name, email", () => {
  ...
  assertEqual(alice.role_schema_id, null);
  assertEqual(alice.module_data, []);
});
...
test("updateUser rejects role_schema_id directly", () => {
  assertThrows(() => updateUser(alice.id, { role_schema_id: "student-role-id" }));
});
...
test("assignRole sets role_schema_id and clears module_data", () => {
  ...
  assertEqual(u.role_schema_id, "student-role-id");
  assertEqual(u.module_data, []);
});
...
test("getUserRole returns role schema", () => {
  ...
  assertEqual(role.role_name, "Student");
});
...
test("fillModuleData stores answer", () => {
  ...
  const entry = u.module_data.find(m => m.module_id === "learning-module-id");
...
test("fillModuleData SINGLE_CHOICE valid", () => {
  ...
  const e = u.module_data.find(m => m.module_id === "year-module-id");
  assertEqual(e.data, ["2nd"]);
});

test("getPendingModules empty after all modules filled", () => {
  // learning already filled, year filled above
  const pending = getPendingModules(alice.id);
...
test("fillModuleDataBatch fills multiple at once", () => {
  ...
  assertEqual(u.module_data.length, 2);
});

test("clearModuleData removes an answer", () => {
  const u = clearModuleData(alice.id, "year-module-id");
  assert(!u.module_data.find(m => m.module_id === "year-module-id"), "year should be cleared");
});
...
test("Student feed includes Teacher Carol (scored tier, score=0 — module linked but no subject overlap)", () => {
  // Carol is a Teacher → isScoredCandidate = true (learning-module-id is shared)
  // But Carol teaches only History while Alice wants Math/Physics → score = 0
  // Tier is still "scored" because the module link exists; score just comes out 0
  const feed = generateFeed(getUser(alice.id), getAllUsers());
...
test("Feed excludes roles not in assigned_roles", () => {
  // Alice is Student; Founder is not in Student.assigned_roles
...
test("calculateScore: [Math,Physics] vs [Math,CS,Chem] ≈ 0.25", () => {
  // Jaccard([Math,Physics],[Math,CS,Chem]) = 1/4 = 0.25; weight=0.6/0.6=1 → 0.25
  const s = calculateScore(getUser(alice.id), getUser(bob.id));
...
test("createRole adds to ROLE_SCHEMAS", () => {
  createRole({ id: "mentor-role-id", role_name: "Mentor", assigned_roles: [] });
  assert(ROLE_SCHEMAS["mentor-role-id"], "role not found after create");
});

test("createRole duplicate id throws", () => {
  assertThrows(() => createRole({ id: "mentor-role-id", role_name: "Mentor2" }));
});

test("updateRole updates fields", () => {
  const r = updateRole("mentor-role-id", { role_name: "Senior Mentor" });
  assertEqual(r.role_name, "Senior Mentor");
});
...
test("Teacher year-module-id hint targets student-role-id", () => {
  const h = getPropagationHints().find(h => h.module_id === "year-module-id" && h.source_role === "Teacher");
...
test("every hint has an action string", () => {
  getPropagationHints().forEach(h => assert(typeof h.action === "string" && h.action.length > 0));
});
...
test("Freelancer feed includes Student (in assigned_roles) at base tier if no module link", () => {
  // Freelancer.require_modules has industry-module-id with target_role: founder
  // Student is in Freelancer.assigned_roles but no require_module targets student
